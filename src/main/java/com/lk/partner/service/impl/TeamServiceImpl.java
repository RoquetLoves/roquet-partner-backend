...
import com.lk.partner.contant.TeamConstant;
import com.lk.partner.exception.BusinessException;
import com.lk.partner.mapper.TeamMapper;
import com.lk.partner.model.entity.Team;
import com.lk.partner.model.entity.User;
import com.lk.partner.model.request.*;
import com.lk.partner.model.vo.TeamUserVo;
import com.lk.partner.model.vo.TeamVo;
import com.lk.partner.service.TeamService;
...
import static com.lk.partner.utils.StringUtils.*;
...
public class TeamServiceImpl extends ServiceImpl<TeamMapper, Team> implements TeamService {
    private static final String SALT = "lk_team";
    private static final String BY_TEAM_IDS = String.format("partner:team:getTeamListByTeamIds:%s", "byTeamIds");
    private static final String TEAMS_KEY = String.format("partner:team:getTeams:%s", "getTeams");
    @Resource
    ...
    public TeamUserVo teamQuery(TeamQueryRequest teamQueryRequest, HttpServletRequest request) {
        ...
        String teamQueryKey = String.format("partner:team:teamQuery:%s", searchText);
        ValueOperations<String, Object> valueOperations = redisTemplate.opsForValue();
        ...
        try {
            ...
            while (true) {
                if (lock.tryLock(0, -1, TimeUnit.MILLISECONDS)) {
                    ...
                    String teamIdKey = String.format("partner:team:getUsersByTeamId:%s", team.getId());
    ...
    public boolean createTeam(TeamCreateRequest teamCreateRequest, User loginUser) {
        ...
        String teamIds = user.getTeamIds();
        RLock lock = redissonClient.getLock("partner:create_team");
        try {
            // 抢到锁并执行
            while (true) {
                if (lock.tryLock(0, -1, TimeUnit.MILLISECONDS)) {
    ...
    public Boolean updateTeam(TeamUpdateRequest teamUpdateRequest, User loginUser) {
        ...
        if (updateStatus) {
            String teamIdKey = String.format("partner:team:getUsersByTeamId:%s", oldTeam.getId());
            deleteRedisKey(teamIdKey);
        }
        return updateStatus;
    }
    ...
    public boolean dissolutionTeam(Long teamId, HttpServletRequest request) {
        ...
        if (dissolutionTeam) {
            String teamIdKey = String.format("partner:team:getUsersByTeamId:%s", teamId);
            deleteRedisKey(teamIdKey);
            redisTemplate.delete(userService.redisFormat(loginUser.getId()));
        }
